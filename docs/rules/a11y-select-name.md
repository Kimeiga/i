...
This maps to WCAG 2.1 success criterion 4.1.2 Name, Role, Value (Level A) and WCAG 2.1 success criterion 3.3.2 Labels or Instructions (Level A). EN 301 549, the harmonised European standard referenced by the European Accessibility Act, incorporates WCAG 2.1 Level AA in full, so a failure here is a failure against the standard European regulators assess against. See [regulatory context](../regulatory-context.md); this is not legal advice.
...
`fixtures/a11y-runtime/select-name/triggering.html`
...
`fixtures/a11y-runtime/select-name/clean.html`
...
```ts
// attest-disable-next-line a11y/select-name -- why this instance is intentional
```

Or for a whole file:

```ts
// attest-disable-file a11y/select-name -- why this file is intentional
```
